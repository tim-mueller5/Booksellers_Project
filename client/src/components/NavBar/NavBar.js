...
const NavBar = ({ user }) => {
    ...
    return(
        <div className='nav-bar'>
            ...
            <Link to="/login" className='login text-uppercase fw-6 fs-16'>{(user != null) ? "Account" : "Login"}</Link>
            <Link to="/create" className='new-user text-uppercase fw-6 fs-16'>{(user != null) ? null : "Create"}</Link>
        </div>
    )
}
